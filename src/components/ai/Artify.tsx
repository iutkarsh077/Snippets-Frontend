          ...
          >
            ...
            <p className="whitespace-pre-wrap">
              {responseFromGemini.map((res: any, index: number) => (
                <div key={index}>
                  <p className="p-2 bg-blue-500 rounded-md mb-3 font-semibold">{res.question.split("    ").map((words: any, index: number)=>(
                    <p key={index}>{words}</p>
                  ))}</p>
                  <p className="pb-5">{<TextGenerateEffect words={res.answer}/>}</p>
